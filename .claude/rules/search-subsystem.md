...
- **Dense encode scheduling is runtime-only.** Foreground query encode and bulk document encode share the daemon-global `GpuEmbeddingDevice` owner for the configured GPU slot. Queries use the shared-session turnstile and are admitted before the next bulk unit; bulk uses worker-owned token-budget batches. CPU is fallback-only when GPU is unavailable or policy forces CPU, never a concurrent overflow lane. Worker count is not a throughput knob: local ONNX clamps `OPTSIDIAN_SEARCH_EMBEDDING_WORKERS=1` to the GPU + CPU-fallback pair, and values above 2 create extra threads that `encodeGpu` / `encodeCpuFallback` do not target. `EmbeddingSpaceKey` is persisted identity; `ResidentModelKey` and `AdmissionPolicy` are runtime/session routing. Batching, scheduling, device policy, and runtime device state never change index identity: batched, multi-device, or heterogeneous builds remain content-addressed MVCC generations with no reindex, projection change, or version bump.
...
| Keep dense batching, scheduling, device policy, and runtime device state outside persisted identity | Bump `INDEX_BUILD_VERSION` / `ANALYZER_VERSION` or force reindex for scheduler/device routing changes |
| Route dense query/bulk encode through the shared GPU owner with CPU fallback-only behavior | Add CPU overflow lanes, treat reserved query sessions as current behavior, or bypass the owner for bulk/query encode |
