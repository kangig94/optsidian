...
export function writePrivateFileSync(filePath: string, data: string | NodeJS.ArrayBufferView, label = "Optsidian file"): void {
...
export function writePrivateFileAtomicSync(filePath: string, data: string | NodeJS.ArrayBufferView, label = "Optsidian file"): void {
  const target = path.resolve(filePath);
...
function assertOwnedByCurrentUser(targetPath: string, stat: fs.Stats, label: string): void {
  ...
  throw new PrivatePathError(
    `${label} at ${targetPath} is owned by uid ${stat.uid}, but the current uid is ${uid}. ` +
    sudoHint()
  );
}
