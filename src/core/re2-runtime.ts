...
import { ensureExistingPrivateFileSync, ensurePrivateDirSync, writePrivateFileAtomicSync, writePrivateFileSync } from "./private-path.js";
...
export async function ensureRe2Runtime(env: NodeJS.ProcessEnv = process.env): Promise<string> {
  ...
  const release = await acquireInstallLock(path.join(path.dirname(targetDir), "install.lock"), RE2_WASM_INSTALL_TIMEOUT_MS);
  try {
    if (isRe2RuntimeInstalled(targetDir)) return targetDir;
    if (fs.existsSync(targetDir)) {
      fs.rmSync(targetDir, { recursive: true, force: true });
    }
    const tempDir = path.join(path.dirname(targetDir), `.install-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}`);
