...
const COMMAND_HELP: Record<ImplementedCommand, CommandHelp> = {
  read: {
    ...
    options: [
      ...
      {
        name: 'max-lines=<n>',
        description: 'Max lines returned; range.end is the actual last line so paging never skips (default: 2000)',
      },
  ...
  search: {
    ...
    options: [
      ...
      {
        name: 'field=<field,...>',
        description: 'Restrict query matching to title, aliases, tags, headings, path, or body',
      },
      ...
      {
        name: 'retrieval=lexical|vector|hybrid',
        description: 'Retrieval source; lexical is the default and never loads the vector model',
      },
    ...
    options: [
      {
        name: 'mode=left|pair',
        description:
          'Comparison mode; left finds notes similar to one note or text, pair scores one note against another',
      },
      ...
      { name: 'min-score=<0..1>', description: 'Minimum Retrieve score threshold (default: 0)' },
      {
        name: 'model=<id>',
        description:
          'Requested embedding model id; text inputs may fall back lexically, note inputs require matching stored vectors',
      },
      {
        name: 'request-json=<json|@file>',
        description: 'Structured request object using the same supported Retrieve-backed subset',
      },
      { name: 'format=text|json', description: 'Output format (default: text)' },
    ],
    notes: [
  ...
  config: {
    ...
    options: [
      ...
      {
        name: 'search.ngram=true|false',
        description: 'Enable Korean 2/3-gram indexing and query expansion; default false',
      },
      {
        name: 'search.partitionBits=<n>',
        description: 'Index partition-scheme bits; default 4. Changing it rebuilds the index',
      },
      ...
      {
        name: 'search.queryCacheSize=<n>',
        description: 'In-memory query-analysis cache entry cap; default 64, 0 disables',
      },
      ...
      {
        name: 'search.daemonIdleMs=<ms>',
        description:
          'Daemon idle shutdown timeout; defaults to 6 hours, while model sessions keep their own idle lifecycle',
      },
      {
        name: 'search.embeddingModel=bge-m3|multilingual-e5-small',
        description: 'Dense embedding model for hybrid/vector retrieval',
      },
  ...
  'open-gui': {
    ...
    options: [
      {
        name: 'vault-path=<path>',
        description:
          'Open a SPECIFIC vault instead of the last-opened one. Normally omit this: bare `optsidian open-gui` is the default usage and opens the last-opened vault. Use vault-path only when you must target a particular vault.',
      },
  ...
  'plugin:install': {
    ...
    options: [
      { name: 'id=<plugin-id>', description: 'Native marketplace plugin id; passed through unchanged' },
      {
        name: 'url=<git-url>',
        description:
          'GitHub/GitHub Enterprise repo shorthand or git URL; prefers a published release, falls back to cloning',
      },
      ...
      { name: 'ref=<git-ref>', description: 'Release tag (preferred) or git branch/commit for url= installs' },
      {
        name: 'auth=true',
        description:
          'Allow GitHub credentials when probing/downloading a private release; off by default for url= installs',
      },
...
export function usagePayload(): {
  ...
  return {
    ...
    preference: {
      ...
      reason:
        'Optsidian keeps routing consistent and avoids shell expansion and quoting bugs when a structured mutation tool is available.',
    },
    help: {
