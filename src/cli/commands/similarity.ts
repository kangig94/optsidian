import { getValue, parsePositiveInt, readRawValueOrFile, readValueOrFile, type ParsedArgs } from '../args.js';
...
export function similarityRequestFromArgs(args: ParsedArgs): SimilarityParams {
  ...
  const provider = getValue(args, 'model') !== undefined ? { model: getValue(args, 'model') } : undefined;
  ...
  return {
    ...
    ...(getValue(args, 'min-score') !== undefined
      ? { minScore: parseScore(getValue(args, 'min-score'), 'min-score') }
      : {}),
  };
}
...
function scopePathsFromArgs(args: ParsedArgs): string[] | undefined {
  ...
  if (raw !== undefined && rawJson !== undefined)
    throw new UsageError('Use either paths=<path,...> or paths-json=<json>, not both');
...
export function retrievePayloadFromSimilarity(request: ReturnType<typeof normalizeSimilarityParams>) {
  ...
  const origin: RetrieveOrigin =
    request.mode === 'left'
      ? request.left?.text !== undefined
        ? 'text'
        : 'note'
      : request.mode === 'pair'
        ? 'pair'
...
function assertRetrieveSupportedSimilarityRequest(request: ReturnType<typeof normalizeSimilarityParams>): void {
  ...
  if (request.projection.version !== DEFAULT_RETRIEVE_PROJECTION_VERSION) {
    throw new UsageError(
      'similarity projection flags are not supported by Retrieve yet; use the default title+body plain projection',
    );
  }
  if (request.mode === 'pair' && (request.left?.text !== undefined || request.right?.text !== undefined)) {
    throw new UsageError(
      'similarity mode=pair requires left=<path> and right=<path>; pair text inputs are not supported by Retrieve yet',
    );
  }
}
