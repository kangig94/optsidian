...
type PinReadableGenerationNotReadyReason =
  'no-active-built-spec' | 'active-generation-mismatched' | 'active-generation-unreadable';
...
export class VectorGenerationPool {
  ...
  async buildStagingGeneration(input: BuildVectorGenerationInput): Promise<BuiltVectorGeneration> {
    ...
    if (existing.status === 'blocked')
      throw new Error(
        `vector generation ${generationId} already exists with conflicting manifest content: ${existing.reason}`,
      );
  ...
  statsForTests(): {
    ...
    return {
      active: Object.fromEntries(
        [...this.activeByKey.entries()].map(([key, generation]) => [key, generation.generationId]),
      ),
      refCounts,
      draining: [...this.generations.values()]
  ...
  private leaseFromPin(pin: VectorPin): ReadableVectorGenerationLease {
    ...
    return {
      ...
      searchVector: (queryVector, candidateK) =>
        Promise.resolve(pin.handle.instance.searchVector(queryVector, candidateK)),
      release: () => {
        if (released) return;
        released = true;
        this.release(pin);
...
function vectorStoreKeysEqual(left: VectorStoreKey, right: VectorStoreKey): boolean {
  return left.vaultStateHash === right.vaultStateHash && left.embeddingSetId === right.embeddingSetId;
}
...
function vectorGenerationMetadataMatchesInput(
  ...
  return (
    vectorStoreKeysEqual(metadata.key, input.paths.key) &&
...
function generationHandleMatchesMetadata(handle: GenerationHandle, metadata: VectorGenerationMetadata): boolean {
  return (
    handle.generationId === metadata.generationId &&
