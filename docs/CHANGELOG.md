...
- Preserve source dataset id directory hierarchy under `IR/`, add balanced `smoke`, stratified `dev`, and `full` IR fixture presets, make quality eval default to fast multicore score-only runs unless `--measure-speed` is requested, and always render warmup/query progress unless `--no-progress` is passed.
- Add a single `search:eval --workers=<n>` control that aligns eval concurrency with daemon search-execution workers while leaving analyzer worker pools at 1 by default.
