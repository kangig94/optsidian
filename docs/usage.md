...
`index status` reports daemon readiness, request metrics, loaded vault snapshot states, and a concurrency section: the owning incarnation (`incarnation`, `epoch`, `pid`), the daemon process RSS, per-pool worker counts with queue depth and the type/vault of any in-flight job, embed-scheduler lane queue depths and the running lane, and query-analysis / search-execution cache hit/miss. `format=json` additionally dumps the full typed `concurrency` object (per-profile pools, embed lanes, and caches). `index rebuild`, `index refresh`, `index warm`, and `index clear` are daemon RPC mutations. Interactive `warm`, `rebuild`, and `refresh` render a single stderr progress bar unless `--no-progress` is passed.
...
optsidian config set search.partitionBits=4
