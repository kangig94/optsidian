...
function assertFallbackEvidence({
  ...
  if (oldSnapshotId) assert.notEqual(expectedSnapshotId, oldSnapshotId, `${reason}: dirty full snapshot must differ from old active`);
  ...
  if (marker) assert.equal(envelopeContainsMarker(envelope, marker), true, `${reason}: persisted envelope contains dirty marker`);
  return envelope;
}
