...
async function assertIncrementalEqualsFull(incremental, full, label) {
  ...
  assert.deepEqual(denseInputsForBuiltSnapshot(incremental), denseInputsForBuiltSnapshot(full), `${label}: dense inputs`);
  ...
  for (const [partitionId, incrementalSegment] of incrementalSegments) {
    ...
    assert.deepEqual(incrementalSegment.documentIds, fullSegment.documentIds, `${label}: segment ${partitionId} documentIds`);
    assert.equal(
    ...
    assert.equal(sha256(incrementalSegment.bytes), incrementalSegment.hash, `${label}: incremental segment ${partitionId} hash`);
  }
  ...
  const incrementalEmbeddingSet = await buildEmbeddingSet({ provider, documents: denseInputsForBuiltSnapshot(incremental) });
  const fullEmbeddingSet = await buildEmbeddingSet({ provider, documents: denseInputsForBuiltSnapshot(full) });
  assert.equal(incrementalEmbeddingSet.embeddingSetId, fullEmbeddingSet.embeddingSetId, `${label}: deterministic dense identity`);
  assert.equal(
...
function applyMutation(vault, documents, operation, random, step) {
  ...
  const rel = choose(paths, random);
  const target = choose(paths.filter((candidate) => candidate !== rel), random) ?? rel;
...
function assertNonUtf8Excluded(built, label) {
  assert.equal(built.documents.some((document) => document.path === "Binary.md"), false, `${label}: non-UTF8 .md excluded`);
}
...
test("AC3 performance gate reparses O(changed docs) on a large link-dense fixture", async (t) => {
  await withDeterministicEmbeddingProvider(async () => {
    ...
    const target = previous.documents.find((document) =>
      /^Doc-\d+\.md$/.test(document.path) &&
      previous.documents.filter((candidate) => candidate.partitionId === document.partitionId).length < previous.documents.length
    );
    assert.ok(target);
