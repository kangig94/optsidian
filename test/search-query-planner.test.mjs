...
test("SearchQueryPlanner imports no pool or worker module and performs no dispatch", () => {
  ...
  assert.equal(importedModules.some((specifier) => /(?:^|\/)(?:pools|worker-pool)\.js$/u.test(specifier)), false);
...
test("SearchQueryPlanner emits deterministic granular full-channel segment tasks without the old two-group cap", () => {
  ...
  const reversed = summarizePlan(planner.plan({
    ...input,
    snapshot: {
      ...input.snapshot,
  ...
  assert.deepEqual(first.tasks.map((task) => task.segments), [
  ...
  assert.deepEqual(first.tasks.map((task) => task.workEstimate), [1, 1, 1, 1]);
  assert.deepEqual(first.tasks.map((task) => task.channels), [
    ["morph", "surface", "ngram"],
    ["morph", "surface", "ngram"],
    ["morph", "surface", "ngram"],
    ["morph", "surface", "ngram"]
