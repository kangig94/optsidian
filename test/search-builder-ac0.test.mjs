...
test("AC0 full-build golden snapshot ids stay byte-identical", async () => {
  ...
  assert.deepEqual(built.segments.map((segment) => segment.hash), [
  ...
  assert.deepEqual(built.segments.map((segment) => sha256(segment.bytes)), built.segments.map((segment) => segment.hash));
  assert.deepEqual(built.documents.map((document) => document.path), ["Alpha.md", "Gamma.md", "Beta.md"]);
  assert.deepEqual(built.linkEdges.map((edge) => [edge.sourcePath, edge.targetPath]), [
    ["Alpha.md", "Beta.md"],
    ["Alpha.md", "Gamma.md"],
    ["Beta.md", "Alpha.md"]
  ]);
...
test("AC0 scan link extraction matches full parse body boundary and skips non-UTF8 documents", async () => {
  ...
  assert.equal(sourceRecord.unresolvedLinks.some((link) => link.rawTarget === "FrontmatterOnly"), false);
  assert.equal(scan.files.includes("Binary.md"), true);
  assert.equal(scan.documents.some((record) => record.path === "Binary.md"), false);

  const built = await buildCanonicalSearchSnapshot({ vaultRoot: vault, analyzer: testAnalyzer(), partitionBits: 1 });
  assert.equal(built.documents.some((document) => document.path === "Binary.md"), false);
  assert.equal(built.linkEdges.some((edge) => edge.targetPath === "FrontmatterOnly.md"), false);
  assert.deepEqual(built.linkEdges.map((edge) => [edge.sourcePath, edge.targetPath]), [
    ["Source.md", "Binary.md"],
