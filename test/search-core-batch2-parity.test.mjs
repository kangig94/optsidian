...
test("AC3 bytes-in-place monolithic path matches the pre-Batch-2 decoded monolithic path", async () => {
  ...
  writeVaultFile(vault, "Research/Calibration Notes.md", "# Calibration Notes\n\nNeedle project alpha calibration body evidence.\n");
  writeVaultFile(vault, "Needle Project.md", "---\ntags: [focus]\n---\n# Needle Project\n\nNeedle project body evidence.\n");
...
function legacyDecodedSearch(built, bm25Stats, search, analysis) {
  ...
  const rerankCandidateSet = {
    ...candidateSet,
    candidates: candidateSet.candidates.filter((candidate) => hits.some((hit) => hit.candidate.candidateId === candidate.candidateId))
  };
  const signals = legacyRankSignals(state, bm25Stats, rerankCandidateSet, hits, search, analysis);
  const ranked = rerankCandidatesWithSignals(analysis.raw, analysis.primaryTerms, hits, search.fields, signals).slice(0, search.limit);
...
function legacyDecodedState(built) {
  ...
  for (const channel of SEARCH_TOKEN_CHANNELS) {
    postingsByChannel[channel] = new Map([...postingsByChannel[channel].entries()].sort(([left], [right]) => left.localeCompare(right)).map(([term, postings]) => [
      term,
...
function legacyRetrieve(state, bm25Stats, analysis, fields, limit, snapshotId) {
  ...
  for (const channel of SEARCH_TOKEN_CHANNELS) {
    ...
    for (const proximityMatch of findProximityMatches(postings, terms, { fieldIds: fields.map((field) => POSITIONAL_FIELD_ID[field]) })) {
...
function legacyScoreChannel(state, bm25Stats, channel, terms, fields) {
  ...
  for (const [docId, fieldsById] of matchedByDocument) {
    ...
    for (const field of fields) {
      ...
      const rawScore = matchedTerms.reduce((sum, term) => sum + legacyBm25TermScore(
        bm25Stats,
        state,
        channel,
        term,
  ...
  return scored.sort((left, right) => right.score - left.score || legacyDocumentKey(state, left.docId).localeCompare(legacyDocumentKey(state, right.docId)));
}
...
function legacyBm25TermScore(bm25Stats, state, channel, term, docId, fieldId, frequency) {
  ...
  if (!corpus || frequency <= 0 || fieldLength <= 0 || documentFrequency <= 0 || corpus.averageFieldLength <= 0) return 0;
...
function legacyFeatureLexicalScore(state, bm25Stats, candidate, fields) {
  ...
  for (const channelRank of candidate.channels) {
    for (const term of channelRank.matchedTerms) {
      for (const field of fields) {
        const fieldId = POSITIONAL_FIELD_ID[field];
        const frequency = state.postingsByChannel[channelRank.channel]
          .get(term)
          ?.find((entry) => entry.docId === candidate.ordinalDocId && entry.fieldId === fieldId)
          ?.positions.length ?? 0;
        ...
        contributions.push({
          ...
          value: legacyBm25TermScore(
            ...
            fieldId,
            frequency
          ) * SEARCH_TOKEN_CHANNEL_WEIGHT[channelRank.channel] * fieldChannelBm25Boost(channelRank.channel, field)
        });
      }
    }
  }
...
function bm25DocumentFrequency(bm25Stats, channel, term, fieldId) {
  return bm25Stats.rows.find((entry) => entry.channel === channel && entry.fieldId === fieldId && entry.term === term)?.documentFrequency ?? 0;
}
...
function legacyBm25SingleTermBounds(state, bm25Stats) {
  ...
  for (const row of bm25Stats.rows) {
    ...
    for (const posting of state.postingsByChannel[row.channel].get(row.term) ?? []) {
      ...
      const score = legacyBm25TermScore(bm25Stats, state, row.channel, row.term, posting.docId, row.fieldId, posting.positions.length);
