...
test("long document snippets fall back to a deep unsampled heading when only frontmatter metadata matches", async () => {
  ...
  writeVaultFile(vault, "DeepFallback.md", [
    "---",
    "aliases:",
    `  - ${marker}`,
    "tags: [deep-fallback]",
  ...
  assert.deepEqual(result.matches[0]?.snippets.map((snippet) => ({
    line: snippet.line,
    text: snippet.text
  })), [
...
test("long document snippets use full frontmatter boundary when closing delimiter is unsampled", async () => {
  ...
  assert.equal(built.documents[0].snippetCorpus.lines.some((line) => line.text.includes(marker)), false);
