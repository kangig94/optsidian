...
test("mcp command_map and write handlers preserve routing, preference guidance, and raw JSON payloads", async () => {
  ...
  try {
    ...
    assert.deepEqual(payload(commandMap).routing.cliOnly, ["read", "search", "grep", "index", "settings", "copy", "mkdir", "open-gui", "update", "frontmatter", "plugin:install"]);
...
test("optsidian-mcp serves tools over stdio protocol", async () => {
  ...
  try {
    ...
    assert.deepEqual(commandMap.structuredContent?.routing?.cliOnly, ["read", "search", "grep", "index", "settings", "copy", "mkdir", "open-gui", "update", "frontmatter", "plugin:install"]);
