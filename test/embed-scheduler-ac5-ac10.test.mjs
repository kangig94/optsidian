...
test("AC5 scheduler lanes run query before save before refresh before rebuild", async () => {
  ...
  const active = scheduler.run("rebuild", async () => {
...
test("AC5 origin=text query encode completes after at most one rebuild slice", async () => {
  ...
  embedding.releaseNextDocument();
  await rebuild;
  assert.deepEqual(embedding.calls.map((call) => call.inputKind), ["document", "query", "document", "document"]);
  await scheduler.close();
});
...
test("AC5 watcher-driven save lets query encode run after one save slice", async () => {
  ...
  try {
    ...
    try {
      ...
      }, context("runtime-save-query"), "query");
      embedding.releaseNextDocument();
      await query;

      const queryCallIndex = embedding.calls.findIndex((call, index) => index > firstSaveCallIndex && call.inputKind === "query");
      ...
      while (embedding.completedDocumentSlices < targetCompletedSlices) {
        if (embedding.pendingDocumentGateCount() === 0) {
          await waitFor(() => embedding.pendingDocumentGateCount() > 0 || embedding.completedDocumentSlices >= targetCompletedSlices);
        }
...
test("AC5 two same-model profiles share one scheduler/model owner and one vector manager with per-key handles", async () => {
  ...
  try {
    ...
    const vault = path.join(root, "vault");
    fs.mkdirSync(vault, { recursive: true });
    const pathsA = vectorStoreCachePaths({ vaultRoot: vault, profileHash: hashA, embeddingSetId: "embedding-shared", env });
    const pathsB = vectorStoreCachePaths({ vaultRoot: vault, profileHash: hashB, embeddingSetId: "embedding-shared", env });
    const spec = makeSpec();
  ...
  const background = (async () => {
    ...
    while (!stopped) {
      try {
        ...
        }, context(`background-${index}`, 1000), "rebuild");
      } catch (error) {
