...
test("AC1 P1 link parser handles escaped pipes markdown titles angle destinations and recursive labels", () => {
  ...
  assert.deepEqual(parsed.unresolvedLinks.map((link) => ({
    kind: link.kind,
...
test("AC1 P1 link projection indexes labels and excludes raw link syntax from every token channel", async () => {
  ...
  writeVaultFile(vault, "Source.md", [
    "Plain [[folder/note]]",
  ...
  for (const expected of ["note", "alias", "label", "disp", "markdown", "bare", "deep", "heading", "card", "한글", "별칭"]) {
  ...
  for (const token of tokens) {
    for (const syntax of ["[[", "]]", "#", "^", "/", "(", ")", "[", "]"]) {
      assert.equal(token.includes(syntax), false, `raw link syntax ${syntax} leaked into token ${JSON.stringify(token)}`);
    }
  }
