...
test("AC3 P2 fusion handles zero weights duplicate merge preference and tie breaking", () => {
  ...
  const fused = fuseCandidateSets([
    candidateSet(lexicalIdentity, [lexicalDuplicate]),
    candidateSet(denseIdentity, [denseDuplicate, beta, alpha])
  ], {
    rawQuery: "needle",
...
test("AC3 P2 deterministic dense retriever propagates denseAgreement through fusion features and rerankScore", async () => {
  ...
  assert.deepEqual(fused.candidates.map((candidate) => candidate.path), ["Far.md", "Near.md"]);
  ...
  const features = [
    featurePayload(nearCandidate, 0),
    featurePayload(farCandidate, 0.05)
  ];
  ...
  const ranked = rerankCandidatesWithSignals(
    ...
    [],
    [
      { document: { id: nearDoc.id, path: nearDoc.path, title: "Near", tags: [] }, score: nearCandidate.retrievalScore },
...
test("AC6 P2 corpusSnapshotId is stable across embedding-model identity changes", async () => {
  ...
  rankingChangedManifest.identityTuple.searchModelIdentity.analyzerIdentity.analyzer.embeddingModel = embeddingModel("model-c");
