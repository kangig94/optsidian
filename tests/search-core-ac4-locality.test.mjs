...
test('AC4 BM25Plus field score matches hand-computed vector and feeds unified lexical score', async () => {
  ...
  const stats = {
    schemaId: 1,
    corpusStats: [{ channel: 'morph', fieldId, documentCount: 2, totalFieldLength: 5, averageFieldLength: 2.5 }],
    rows: [{ channel: 'morph', fieldId, term: 'alpha', documentFrequency: 1 }],
    hash: 'fixture',
  };
...
test('AC4 exact-priority lambda dominates max-term multi-channel body stuffing', async () => {
  ...
  const ranked = rerankCandidatesWithSignals(
    ...
    new Map([
      [
        'Stuffed.md',
        { ...baseSignal, lexicalScore: bound.lexicalBound, identityScore: 0, proximityScore: bound.proximityBound },
      ],
  ...
  assert.deepEqual(
    ...
    ['Title.md', 'Alias.md', 'Needle.md', 'Stuffed.md'],
  );
});
