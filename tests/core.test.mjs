...
test('AC6 search query and Hangul ngram analysis are length-bounded', async () => {
  ...
  assert.throws(
    () => normalizeSearchParams({ tags: ['project'], fields: ['body'] }),
    /field=<field> requires query=<text>/,
  );
  assert.throws(
    () => normalizeSearchParams({ query: 'alpha', retrieval: 'invalid' }),
    /retrieval must be lexical, vector, or hybrid/,
  );
  assert.throws(
    () => normalizeSearchParams({ query: 'alpha', coverage: 'invalid' }),
    /coverage must be full or bounded/,
  );
  assert.throws(
    () => normalizeSearchParams({ query: 'alpha', budget: { work: 1 } }),
...
test('similarity contract normalizes vector request', async () => {
  ...
  assert.throws(
    () => normalizeSimilarityParams({ mode: 'pair', left: { path: 'a.md' } }),
    /mode=pair requires left and right/,
  );
...
test('reranker keeps English polarity terms eligible for metadata coverage', async () => {
  ...
  const ranked = rerankCandidatesWithSignals(
    ...
    undefined,
    new Map([['SciFact/not-title.md', rankSignal({ lexicalScore: 1, coverageTerms: 1, coverageFieldScore: 1 })]]),
  );
...
test('intl search analyzer segments CJK text for lexical search', async () => {
  ...
  const envOverSettings = resolveSearchAnalyzer(
    { OPTSIDIAN_SEARCH_EXTRA_LANGS: '' },
    { search: { extraLangs: ['ko'] } },
    runtime,
  );
  assert.deepEqual(envOverSettings.identity.declaredAnalyzers, []);
  assert.deepEqual(
    resolveSearchAnalyzer({ OPTSIDIAN_SEARCH_ANALYZER: 'kiwi' }, { search: { analyzer: 'intl' } }, runtime).identity
      .activeAnalyzers,
    ['ko'],
  );
  assert.equal(
...
test('kiwi analyzer manager supports non-blocking and blocking modes', async () => {
  ...
  try {
    ...
    const blocking = await manager.withAnalyzerLease({}, ['ko'], { wait: true, installIfMissing: true }, (lease) =>
      lease.analyzer?.tokens('한국어'),
    );
...
test('vault access registry keeps recent realpaths only', async () => {
  ...
  assert.deepEqual(
    state.vaults.map((entry) => entry.realpath),
    [thirdReal, secondReal],
  );
...
test('private path helper rejects symlinked private targets', async () => {
  ...
  assert.throws(() => ensurePrivateDirSync(linkedDir, 'Optsidian linked test directory'), /must not be a symlink/);
  assert.throws(
...
test('core frontmatter reads and mutates structured YAML while preserving body', async () => {
  const vault = tempVault();
  const { addFrontmatterValue, deleteFrontmatter, readFrontmatter, removeFrontmatterValue, setFrontmatter } =
    await core();
  fs.writeFileSync(
...
test('core frontmatter creates blocks and rejects unsafe YAML shapes', async () => {
  ...
  assert.throws(
    () => setFrontmatter(vault, { path: 'duplicate.md', key: 'b', value: true }),
  ...
  assert.throws(
    () => addFrontmatterValue(vault, { path: 'scalar-list.md', key: 'tags', value: 'alpha' }),
    /not a list/,
  );
...
test('core validates adapter-independent numeric parameters', async () => {
  ...
  assert.throws(
    () => readVaultFile(vault, { path: 'note.md', lines: { start: 3, end: 2 } }),
    /lines\.end must be >= lines\.start/,
  );
  ...
  await assert.rejects(
    () =>
      editVaultFile(vault, {
        path: 'note.md',
        selector: { kind: 'range', value: { start: 2, end: 1 } },
        replacement: 'x',
      }),
