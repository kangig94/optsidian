...
test('search CLI request payload carries explicit retrieval and bounded coverage budgets', async () => {
  ...
  const budgeted = request(['needle', 'budget-work=20', 'budget-shards=3', 'budget-time-ms=40']);
...
test('search CLI rejects invalid retrieval, coverage, and full budget combinations', async () => {
  ...
  assert.throws(
    () => request(['needle', 'retrieval=vector', 'field=title']),
    /field=<field> is not supported with retrieval=vector/,
  );
  for (const budgetFlag of ['budget-work=1', 'budget-shards=1', 'budget-time-ms=1']) {
    assert.throws(() => request(['needle', 'coverage=full', budgetFlag]), /search budgets require coverage=bounded/);
  }
});
...
test('top-level help includes native passthrough error verbatim when command listing fails', () => {
  ...
  const result = run(['--help'], {
    env: { ...env, FAKE_OBSIDIAN_HELP_FAIL: 'Start the Obsidian GUI to use native help.' },
  });
  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /Native passthrough:/);
...
test('plugin:install url installs from a git subdirectory and reloads the active vault', async () => {
  ...
  const result = await runPluginInstallDirect(
    [`url=${url}`, 'ref=main', 'dir=dist/obsidian-plugin', `vault-path=${vault}`, 'enable', 'format=json'],
    { env },
  );
  assert.equal(result.status, 0, result.stderr);
...
test('plugin:install url installs from a GitHub Enterprise-style release API', async () => {
  ...
  try {
    ...
    const result = await runPluginInstallDirect([`url=${sourceUrl}`, `vault-path=${vault}`, 'enable', 'format=json'], {
...
test('plugin:install normalizes scheme-less GitHub URLs', async () => {
  ...
  assert.equal(
    normalizeGitSource('github.example.test/user/sample-plugin'),
    'https://github.example.test/user/sample-plugin',
  );
...
test('policy table does not implement native-sufficient commands', async () => {
  ...
  for (const command of policy.implementedCommands()) {
    assert.equal(
      ...
      false,
      `${command} must not be both implemented and native-sufficient`,
    );
  }
});
...
test('destructive native command help never executes the command', () => {
  ...
  assert.equal(result.status, 0, result.stderr);
  calls = fs
    .readFileSync(passthrough.log, 'utf8')
    .trim()
    .split('\n')
    .map((line) => JSON.parse(line));
...
test('search ranks notes and renders CLI output', async (t) => {
  ...
  const warm = run(['index', 'warm', `vault-path=${vault}`, 'format=json'], {
    env: { ...env, ...coralEnv, XDG_CACHE_HOME: cache },
  });
  assert.equal(warm.status, 0, warm.stderr);

  let result = run(['search', `vault-path=${vault}`, 'query=project alpha', 'format=json', 'limit=2', 'debug=true'], {
    env: { ...env, ...coralEnv, XDG_CACHE_HOME: cache },
  });
  assert.equal(result.status, 0, result.stderr);
  const payload = JSON.parse(result.stdout);
  ...
  result = run(['search', `vault-path=${vault}`, 'query=project alpha', 'path=Projects', 'limit=2'], {
    env: { ...env, ...coralEnv, XDG_CACHE_HOME: cache },
  });
  assert.equal(result.status, 0, result.stderr);
...
test('similarity command parses retrieve sugar', async () => {
  ...
  const pair = normalizeSimilarityParams(
    similarityRequestFromArgs(
      parseArgs(['similarity', 'mode=pair', 'left=Projects/A.md', 'right=Projects/B.md', 'format=json']),
    ),
  );
  ...
  assert.throws(
    () =>
      retrievePayloadFromSimilarity(
        normalizeSimilarityParams(similarityRequestFromArgs(parseArgs(['similarity', 'mode=global', 'path=Projects']))),
      ),
    ...
    () =>
      retrievePayloadFromSimilarity(
        normalizeSimilarityParams(
          similarityRequestFromArgs(
            parseArgs(['similarity', 'mode=left', 'left=Projects/A.md', 'paths=Projects/A.md,Projects/B.md']),
          ),
        ),
      ),
  ...
  assert.throws(
    () =>
      retrievePayloadFromSimilarity(
        normalizeSimilarityParams(
          similarityRequestFromArgs(
            parseArgs(['similarity', 'mode=left', 'left=Projects/A.md', 'path-glob=LLM-Wiki/*/index.md']),
          ),
        ),
      ),
  ...
  assert.throws(
    () =>
      retrievePayloadFromSimilarity(
        normalizeSimilarityParams(
          similarityRequestFromArgs(parseArgs(['similarity', 'mode=left', 'left=Projects/A.md', 'field=title'])),
        ),
      ),
  ...
  assert.throws(
    () =>
      retrievePayloadFromSimilarity(
        normalizeSimilarityParams(
          similarityRequestFromArgs(parseArgs(['similarity', 'mode=pair', 'left-text=alpha', 'right=Projects/B.md'])),
        ),
      ),
  ...
  const structured = normalizeSimilarityParams(
    similarityRequestFromArgs(parseArgs(['similarity', `request-json=${requestJson}`, 'format=json'])),
  );
  ...
  assert.throws(
    () =>
      normalizeSimilarityParams(
        similarityRequestFromArgs(parseArgs(['similarity', 'path=Projects', 'path-glob=LLM-Wiki/*/index.md'])),
      ),
    /Use only one/,
  );
});
...
test('index mutation rendering stays stable', async () => {
  ...
  assert.equal(
    renderIndexResult({ ok: true, command: 'index', action: 'refresh', rebuilt: false }),
    'Index already fresh.\n',
  );
    ...
    renderIndexResult({
      ...
      vaults: [
        { vault: '/vault-a', state: 'ready', snapshotId: 'abc123', updatedAt: '2026-01-01T00:00:00.000Z' },
        {
          vault: '/vault-b',
          state: 'loading',
          progress: { phase: 'building', completed: 1, total: 2, current: 'note.md' },
        },
      ],
    }),
    [
...
test('frontmatter command reads and mutates structured metadata', () => {
  ...
  let result = run(['frontmatter', 'set', 'path=note.md', 'key=aliases', `value-json=@${values}`, 'format=json'], {
    env,
  });
  assert.equal(result.status, 0, result.stderr);
