...
test('mcp command_map and write handlers preserve routing, preference guidance, and raw JSON payloads', async () => {
  ...
  try {
    ...
    assert.deepEqual(payload(commandMap).routing.cliOnly, [
      'read',
      'search',
      'similarity',
      'grep',
    ...
    assert.deepEqual(payload(commandMap).routing.mcpTools, [
      'command_map',
      'command_run',
      'write',
      'edit',
    ...
    assert.match(
      payload(commandMap).preference.reason,
...
test('mcp config and native vault resolution use fake obsidian', async () => {
  ...
  assert.equal(
    resolveObsidianVaultRootWithFallback({ vault: 'Work', fallbackPath: path.join(dir, 'missing'), env }),
    vault,
  );
  const calls = fs
...
test('mcp fixed vault path is used when native resolve fails', async () => {
  ...
  assert.throws(
    () => resolveObsidianVaultRootWithFallback({ fallbackPath: path.join(dir, 'missing'), env }),
...
test('optsidian-mcp serves tools over stdio protocol', async () => {
  ...
  try {
    ...
    assert.deepEqual(commandMap.structuredContent?.routing?.cliOnly, [
      'read',
      'search',
      'similarity',
      'grep',
    ...
    assert.deepEqual(commandMap.structuredContent?.routing?.mcpTools, [
      'command_map',
      'command_run',
      'write',
      'edit',
    ...
    assert.deepEqual(commandMap.structuredContent?.routing?.nativeCommands, [
      'files',
      'links',
      'version',
      'dev:console',
