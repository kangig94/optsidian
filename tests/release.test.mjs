...
function createToolBin(dir, options = {}) {
  ...
  if (options.codexLog) {
    writeFakeClient(binDir, 'codex');
  }
  if (options.claudeLog) {
    writeFakeClient(binDir, 'claude');
  }
  if (options.obsidianLog) {
    writeFakeObsidian(binDir);
  }
  return binDir;
}
...
function writeFakeClient(binDir, name) {
...
function writeFakeObsidian(binDir) {
...
function makeReleaseBundle(dir, version, options = {}) {
  ...
  fs.writeFileSync(
    ...
    [optsidianAsset, optsidianMcpAsset]
      .map((asset) => `${sha256(asset.filePath)}  ${asset.name}`)
      .join('\n')
      .concat('\n'),
  );
...
function writeAttestationAsset(dir, tag) {
  ...
  fs.writeFileSync(
    asset.filePath,
    `${JSON.stringify({ mediaType: 'application/vnd.dev.sigstore.bundle+json', tag }, null, 2)}\n`,
  );
  return asset;
}
...
async function startReleaseServer(releases, latestTag) {
  ...
  const byAsset = new Map(releases.flatMap((release) => release.assets.map((asset) => [asset.name, asset.filePath])));
...
test('package-release script creates the expected release asset contract', () => {
  ...
  assert.deepEqual(
    ...
    [`optsidian-${tag}`, `optsidian-mcp-${tag}`, `checksums-${tag}.txt`],
  );
  assert.deepEqual(
    fs.readdirSync(outDir).sort(),
    [`checksums-${tag}.txt`, `optsidian-${tag}`, `optsidian-mcp-${tag}`].sort(),
  );
});
...
test('network auth token lookup disables interactive credential prompts', async () => {
  ...
  try {
    ...
    const calls = fs
      .readFileSync(credentialLog, 'utf8')
      .trim()
      .split('\n')
...
test('official update release fetch and attestation verify do not use GitHub credentials', async () => {
  ...
  try {
    ...
    assert.equal(result.status, 0, result.stderr);
    assert.equal(
      server.requestRecords.every((record) => record.authorization === undefined),
      true,
    );
    ...
    assert.equal(
      ghCalls.every((call) => call.githubToken === '' && call.ghToken === ''),
      true,
    );
    assert.equal(
      ghCalls.every((call) => call.ghConfigDir),
      true,
    );
  } finally {
    await server.close();
...
test('install.sh installs the latest release and succeeds without MCP clients', async () => {
  ...
  try {
    ...
    const curlCalls = fs
      ...
      .map((line) => JSON.parse(line));
    assert.equal(
      curlCalls.some((args) => args.some((arg) => /^Authorization:/i.test(arg))),
      false,
    );
    assert.equal(
