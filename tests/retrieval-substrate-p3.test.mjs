...
test('AC2 P3 link graph sidecar hash/load/view accessors are deterministic', async () => {
  ...
  assert.deepEqual(
    view.outlinks(indexId).map((edge) => edge.targetPath),
    ['AliasTarget.md', 'Embeds/Card.md', 'docs/Markdown Target.md', 'folder/Target.md'],
  );
  assert.deepEqual(
    view.inlinks(targetId).map((edge) => edge.sourcePath),
    ['Index.md'],
  );
  assert.deepEqual(
    view.neighbors(indexId).map((neighbor) => neighbor.path),
    ['AliasTarget.md', 'Embeds/Card.md', 'docs/Markdown Target.md', 'folder/Target.md'],
  );
  ...
  try {
    ...
    assert.deepEqual(
      pin.view.outlinks(indexId).map((edge) => edge.targetPath),
      view.outlinks(indexId).map((edge) => edge.targetPath),
    );
    assert.deepEqual(
      pin.view.inlinks(targetId).map((edge) => edge.sourcePath),
      ['Index.md'],
    );
    assert.deepEqual(
      pin.view.neighbors(indexId).map((neighbor) => neighbor.documentId),
      view.neighbors(indexId).map((neighbor) => neighbor.documentId),
    );
  } finally {
    store.release(pin);
  }
...
test('AC2 P3 link graph GC roots protect active in-flight loaded retained and collect orphans', async () => {
  ...
  assert.equal(
    fs.existsSync(linkGraphSidecarPath(paths, builtB.linkGraphId)),
    true,
    'active link graph must survive GC',
  );
  assert.equal(
    fs.existsSync(linkGraphSidecarPath(paths, builtA.linkGraphId)),
    true,
    'loaded link graph must survive GC',
  );
  ...
  await eventually(() => {
    assert.equal(
      fs.existsSync(linkGraphSidecarPath(paths, orphan.linkGraphId)),
      false,
      'orphan link graph must be collected',
    );
  });
  ...
  await eventually(() => {
    assert.equal(
      fs.existsSync(linkGraphSidecarPath(paths, releaseOrphan.linkGraphId)),
      false,
      'release orphan link graph must be collected',
    );
  });
  assert.equal(
    fs.existsSync(linkGraphSidecarPath(paths, builtA.linkGraphId)),
    true,
    'loaded link graph remains protected after pin release',
  );
  ...
  await eventually(() => {
    assert.equal(
      fs.existsSync(linkGraphSidecarPath(retainedPaths, retainedOrphan.linkGraphId)),
      false,
      'retained-store orphan link graph must be collected',
    );
  });
  assert.equal(
    fs.existsSync(linkGraphSidecarPath(retainedPaths, builtA.linkGraphId)),
    true,
    'retained link graph must survive GC',
  );
  assert.equal(
    fs.existsSync(linkGraphSidecarPath(retainedPaths, builtB.linkGraphId)),
    true,
    'active retained-store link graph must survive GC',
  );
});
...
test('AC4 P3 link adjacency retriever propagates linkAgreement through fusion features and rerankScore', async () => {
  ...
  const linkGraph = createLinkGraphView(
    buildLinkGraphSidecar({
      corpusSnapshotId: built.corpusSnapshotId,
      edges: built.linkEdges,
    }),
  ...
  assert.deepEqual(
    lexical.candidates
      .filter((candidate) => candidate.path === 'Linked.md' || candidate.path === 'A-Unlinked.md')
  ...
  const features = [featurePayload(linked, 1), featurePayload(unlinked, 1)];
  ...
  const ranked = rerankCandidatesWithSignals(
    ...
    [
      {
        document: { id: unlinked.documentId, path: unlinked.path, title: 'A-Unlinked', tags: [] },
        score: unlinked.retrievalScore,
      },
      {
        document: { id: linked.documentId, path: linked.path, title: 'Linked', tags: [] },
        score: linked.retrievalScore,
      },
    ],
    undefined,
    signals,
