...
function assertNoPersistedRecordForbiddenKeys(serialized, label) {
  ...
  assert.doesNotMatch(
    serialized,
    /\[document\.body\]/u,
    `${label} must not contain the old [document.body] field-text marker`,
  );
}
...
function assertNoPlaintextOutsideSnippetText(value, phrases, label, pathParts = []) {
  if (typeof value === 'string') {
    const isSnippetText =
      pathParts[pathParts.length - 1] === 'text' && pathParts.includes('snippetCorpus') && pathParts.includes('lines');
    if (!isSnippetText) {
      for (const phrase of phrases) {
        assert.equal(
          value.includes(phrase),
          false,
          `${label} leaked ${JSON.stringify(phrase)} at ${pathParts.join('.')}`,
        );
      }
    }
    return;
  ...
  if (Array.isArray(value)) {
    value.forEach((child, index) =>
      assertNoPlaintextOutsideSnippetText(child, phrases, label, [...pathParts, String(index)]),
    );
    return;
  }
  for (const [key, child] of Object.entries(value)) {
...
function snippetTextOccurrenceCount(records, needle) {
  return records.reduce(
    (sum, record) =>
      sum +
      (record.snippetCorpus?.lines ?? []).reduce(
...
test('AC7 persisted worker bytes and decoded segments carry no plaintext body copies', async () => {
  ...
  const phrases = ['private body alpha phrase', 'private body beta phrase'];
  writeVaultFile(
    vault,
    'Privacy.md',
    ['---', 'tags: [privacy]', '---', '# Privacy Title', '', phrases[0], phrases[1]].join('\n'),
  );
  ...
  assertPersistedRecordsArePrivate(
    envelope.documents,
    JSON.stringify(envelope.documents),
    phrases,
    'on-disk envelope documents',
  );
  ...
  for (const segment of handle.segments) {
    ...
    assert.equal(
      decoded.fieldTexts?.some((fieldText) => fieldText.fieldId === POSITIONAL_FIELD_ID.body),
      false,
    );
  ...
  assert.throws(
    () =>
      rerankCandidatesWithSignals(
        ...
        [hit],
        undefined,
        new Map([['doc-alpha', { ...complete, lexicalScore: undefined }]]),
      ),
    /lexicalScore/,
  );
