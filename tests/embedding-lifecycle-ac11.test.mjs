...
function pickChangedDocument(envelope) {
  ...
  const candidate = envelope.documents.find(
    (document) =>
...
test('AC11 watcher save debounce publishes a coherent lexical revision and dense generation without Refresh/Rebuild', async () => {
  ...
  try {
    ...
    const initialSegmentBytes = new Map(
      [...initialSegments.values()].map((hash) => [hash, fs.readFileSync(path.join(paths.segmentsDir, hash))]),
    );
    ...
    const unchangedPartitions = [...initialSegments.keys()].filter(
      (partitionId) => !affectedPartitions.has(partitionId),
    );
