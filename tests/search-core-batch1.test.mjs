...
test('canonical segment codec round-trips to byte-identical canonical bytes', () => {
  ...
  assert.deepEqual(
    decoded.postings.map((posting) => [posting.term, posting.fieldId, posting.docId, posting.positions]),
    [
      ['alpha', 1, 1, [0, 4]],
      ['beta', 0, 2, [1, 3, 5]],
    ],
...
test('positional phrase matching requires consecutive positions', () => {
  ...
  assert.deepEqual(findPhraseMatches(postings, ['alpha', 'beta']), [{ docId: 7, fieldId: body, starts: [0] }]);
...
test('BM25 computes per-field statistics and keeps channel boost separate from fusion weight', () => {
  ...
  const expectedScore =
    (expectedIdf * (0.5 + expectedTf * (1.2 + 1))) / (expectedTf + 1.2 * (1 - 0.75 + 0.75 * (3 / 2)));
