...
function syntheticBody(index) {
  ...
  return Array.from(
    { length: 80 },
    (_, repeat) => `${english} ${korean} doc-${index} repeat-${repeat} topic-${index % 11}`,
  ).join('\n');
}
...
function createSyntheticVault(documentCount) {
  ...
  for (let index = 0; index < documentCount; index += 1) {
    writeVaultFile(
      ...
      ['---', `tags: [tag-${index % 5}, ac5]`, '---', `# Parallel Build ${index}`, '', syntheticBody(index)].join('\n'),
    );
  }
  return vault;
}
...
async function buildWithIndexWorkers(vault, workers) {
  ...
  try {
    ...
    const built = await pools.throughputAnalyzer.buildSnapshot(
      vault,
      3,
      {
