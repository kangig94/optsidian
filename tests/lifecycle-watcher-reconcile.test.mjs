...
test('AC4 reconcile prunes only missing watchers in the reconciled subtree', async () => {
  ...
  try {
    ...
    assert.equal(
      fake.closed.includes(bDead),
      false,
      'missing watched directory outside reconciled subtree stayed open',
    );
    assert.equal(
      fake.closed.includes(bChild),
      false,
      'missing watched descendant outside reconciled subtree stayed open',
    );
  } finally {
    producer.close();
  }
...
test('AC4 moved-in trees with pre-existing markdown are marked by enumeration', async () => {
  ...
  try {
    ...
    assert.deepEqual(marks.map((mark) => mark.path).sort(), ['Tree/Root.md', 'Tree/Sub/Nested.md']);
...
test('AC4 reconcile preserves delete marks and dotdir skip behavior', async () => {
  ...
  try {
    ...
    assert.equal(typeof rootListener, 'function');
    assert.equal(
      [...fake.listeners.keys()].some((dir) => dir.includes(`${path.sep}.hidden`)),
      false,
    );
